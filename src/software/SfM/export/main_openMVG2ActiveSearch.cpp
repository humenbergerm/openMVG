        ...
        const std::string & sOutListFile)  //Output Bundler list.txt file
{
  ...
  else
  {
    ...
    std::string rel_path = stlplus::folder_to_relative_path(stlplus::folder_part(sOutListFile), sfm_data.s_root_path);
    //std::cout << stlplus::folder_part(sOutListFile) << std::endl << sfm_data.s_root_path << std::endl << rel_path << std::endl << std::endl;
         ...
         iter != sfm_data.GetViews().end(); ++iter)
    {
      ...
      if (isPinhole(iterIntrinsic->second->getType()))
      {
        ...
        //  << " 0 " << focal << os.widen('\n');
        osList << rel_path + view->s_Img_path << " 0 " << focal << os.widen('\n');
      }
      else
      {
